...
### Rule: Cache raw Google API responses under .tmp for debugging and iteration

**Directive:** When debugging or building a feature that calls a billed Google Maps Platform API (Directions, Street View Static, Maps Static), ALWAYS prefer reusing a cached raw response over re-querying the live API. This project's HTTP client layer (net::cached_fetch, wired into directions.rs/streetview.rs/maps.rs) caches raw response bytes under the directory named by the SVMM_HTTP_CACHE_DIR env var when it is set, keyed by a hash of the request URL with the API key stripped. Point it at a .tmp/ subdirectory (e.g. SVMM_HTTP_CACHE_DIR=.tmp/http_cache) for local debugging runs and for the ignored e2e tests in tests/pipeline.rs, so repeated iteration during a session does not repeatedly bill the same API calls. Delete the cache directory to force a fresh real check against the live API.

**Rationale:** Iterating on this feature required many real, billed API calls (Directions + hundreds of Street View images + Maps Static per run) just to visually re-check a compositing change — the same route was re-fetched from scratch on every iteration even though the underlying API response never changed. A URL-keyed raw-response cache under the already-gitignored .tmp/ directory removes that repeated cost with no risk to production behavior, since it only activates when the env var is explicitly set.

...
- **What mechanism enforces it, and at what boundary.** A local pre-commit hook only stops a normal local git
  workflow -- it does not stop a commit made via the GitHub web UI/API, `git commit --no-verify`, or from a machine
...
- A markdown artifact with no structured sub-content (plain prose, e.g. a one-off `.context/analysis/*.md` body) --
  ...
  need its own schema.
- A skill that only *reads* artifacts (e.g. `docs-check`) rather than creating/updating them.

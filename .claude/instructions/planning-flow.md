...
- It is large enough to warrant its own branch and PR for reasons other than convenience.
...
4. **Implementation.** Only now write code. Follow the usual discipline (TDD, verify, feature branch, PR). This repo is
   on **GitHub**: author the PR via `gh pr create`. Capture any resulting architectural decision as an ADR via
   `adr-capture` (see ADR-worthiness below).
...
- **Scrub before fan-out.** Findings and plans can quote logs, config, or sample data. Before dispatching them to
  reviewer subagents or models, scrub secrets (API keys, tokens) and any personal data that may appear in test
  fixtures or sample routes.

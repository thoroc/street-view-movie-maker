...
| `qmd-embed-thoroc` | commit touches `.claude/**/*.md` | `qmd embed -c thoroc` — refresh `vec`/`hyde` embeddings for changed instructions/skills |

So a normal commit touching `.claude/**/*.md` leaves both keyword and semantic search current with no manual step. If the index appears stale (search misses a file you know exists), run manually:
